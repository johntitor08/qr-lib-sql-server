...
async function submitAuth() {
  ...
  btn.disabled = true;
  btn.textContent = _authTab === 'login' ? 'Giriş yapılıyor...' : 'Kayıt yapılıyor...';
...
async function signOut() {
  const ok = await appConfirm({
    ...
    okLabel: 'Çıkış Yap', okClass: 'btn-danger', cancelLabel: 'İptal',
  });
  if (!ok) return;
  clearToken();
  books = []; highlights = []; loans = []; demoMode = false; _currentUser = null;
...
async function loadBooks(page = 1) {
  ...
  if (grid) grid.innerHTML = [1,2,3,4].map(() =>
  ...
  try {
    ...
    renderAll();
  } catch(e) { toast('Kitaplar yüklenemedi: ' + e.message, 'error'); }
}

async function saveBook() {
  const book = getFormData();
  if (!book.title || !book.author) { toast('Kitap adı ve yazar zorunlu', 'error'); return; }
  if (book.isbn && !validateISBN(book.isbn)) { toast('Geçersiz ISBN formatı', 'error'); return; }
  ...
  try {
    if (demoMode) {
      ...
      const saved = await apiFetch('/books', { method: 'POST', body: JSON.stringify(book) });
      books.unshift(saved); clearForm(); renderAll();
...
async function deleteBook(bookId) {
  ...
  const ok = await appConfirm({
    icon: '🗑️', title: 'Kitabı Sil',
    msg: book ? `"<strong>${esc(book.title)}</strong>" kalıcı olarak silinecek.` : 'Bu kitabı silmek istiyor musunuz?',
    okLabel: 'Evet, Sil', okClass: 'btn-danger',
  });
  if (!ok) return;
  try {
    if (demoMode) { books = books.filter(b => b.id !== bookId); saveDemo(); }
    else { await apiFetch('/books/' + bookId, { method: 'DELETE' }); books = books.filter(b => b.id !== bookId); }
...
async function loadHighlights() {
  ...
  } catch(e) { console.warn('Alıntılar yüklenemedi:', e.message); }
}
...
async function saveHighlight() {
  ...
  try {
    let hl;
    if (demoMode) {
      hl = { id: 'hl_' + Date.now(), bookId, ...payload, createdAt: new Date().toISOString() };
      highlights.unshift(hl); saveDemo();
    } else {
      hl = await apiFetch('/highlights', { method: 'POST', body: JSON.stringify(payload) });
  ...
  } catch(e) { toast('Alıntı kaydedilemedi: ' + e.message, 'error'); }
}
...
async function deleteHighlight(id) {
  ...
  if (demoMode) { saveDemo(); }
    ...
    try { await apiFetch('/highlights/' + id, { method: 'DELETE' }); }
...
async function loadLoans() {
  ...
  } catch(e) { console.warn('Ödünçler yüklenemedi:', e.message); }
}
...
async function saveLoan() {
  ...
  try {
    if (demoMode) {
      const loan = { id: 'ln_' + Date.now(), ...payload,
        bookName: payload.book_name, returned_date: null, created_at: new Date().toISOString() };
      loans.unshift(loan); saveLoanStorage();
    } else {
      const loan = await apiFetch('/loans', { method: 'POST', body: JSON.stringify(payload) });
      loan.bookName = loan.book_name;
      loans.unshift(loan); saveLoanStorage();
    }
    closeModal('addLoanModal'); renderLoans(); updateLoansBadge();
    ['loan_book_name','loan_borrower','loan_contact','loan_notes'].forEach(id => {
      const el = document.getElementById(id); if (el) el.value = '';
    });
    toast('Ödünç kaydı oluşturuldu!', 'success');
  } catch(e) { toast('Ödünç kaydedilemedi: ' + e.message, 'error'); }
}
...
async function markLoanReturned(id) {
  ...
  if (demoMode) { saveLoanStorage(); }
  else {
    try { await apiFetch('/loans/' + id + '/return', { method: 'PATCH' }); saveLoanStorage(); }
    catch(e) { toast('Hata: ' + e.message, 'error'); return; }
  }
  renderLoans(); updateLoansBadge(); toast('İade kaydedildi ✅', 'success');
}
...
async function deleteLoan(id) {
  ...
  if (demoMode) { saveLoanStorage(); }
    ...
    try { await apiFetch('/loans/' + id, { method: 'DELETE' }); saveLoanStorage(); }
    catch(e) { toast('Hata: ' + e.message, 'error'); return; }
  }
  renderLoans(); updateLoansBadge(); toast('Kayıt silindi');
}
...
async function renderAdminPanel() {
  if (!_currentUser || _currentUser.email !== ADMIN_EMAIL) {
    showPage('dashboard'); toast('Bu sayfaya erişim yetkiniz yok', 'error'); return;
  }
...
async function adminApproveUser(userId, email) {
  try {
    ...
    toast(`${email} onaylandı ✅`, 'success'); renderAdminPanel();
  } catch(e) { toast('Hata: ' + e.message, 'error'); }
}

async function adminRevokeUser(userId, email) {
  const ok = await appConfirm({ icon:'⚠️', title:'Erişimi Kaldır', msg:`${email} kullanıcısının erişimi kaldırılacak.`, okLabel:'Kaldır', okClass:'btn-danger' });
  if (!ok) return;
  try {
    await apiFetch('/users/' + userId + '/revoke', { method: 'PATCH' });
    toast(`${email} erişimi kaldırıldı`, 'info'); renderAdminPanel();
  } catch(e) { toast('Hata: ' + e.message, 'error'); }
}

async function adminRejectUser(userId, email) {
  const ok = await appConfirm({ icon:'🗑', title:'Reddet', msg:`${email} kullanıcısını reddet ve sil?`, okLabel:'Sil', okClass:'btn-danger' });
  if (!ok) return;
  try {
    await apiFetch('/users/' + userId, { method: 'DELETE' });
    toast(`${email} reddedildi`, 'info'); renderAdminPanel();
  } catch(e) { toast('Hata: ' + e.message, 'error'); }
}